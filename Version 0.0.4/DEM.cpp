...
void DEM::Cycle() {
	...
	for (auto& B : bodies) {
		...
		for (auto& I : B->inter) {
			auto Il = I.lock(); assert(Il);
			auto Ilb1 = Il->body1.lock(); assert(Ilb1);
			auto Ilb2 = Il->body2.lock(); assert(Ilb2);
			if (B->id == Ilb1->id) {
				B->force += -Il->normalForce * Il->unitNormal;
				B->force += -Il->shearForce * Il->unitShear;
				B->moment += Il->shearForce * (Il->contact - B->pos).norm();
		...
		//get all forces and moments on the node		
		Vec2d f = Vec2d::Zero();
		f += B->force;
		f += B->mass * gravity;
		f += applyBorderForce(B);

		double m = 0;
		m += B->moment;

		// calculate accelerations from forces and moments
		Vec2d linAccel = Vec2d::Zero();
		Vec2d auxVel = { 0,0 };
		for (int i = 0; i < 2; ++i) {
			if (B->vel[i] > 0) auxVel[i] = 1; if (B->vel[i] < 0) auxVel[i] = -1;
			linAccel[i] = ((f[i] - localDamping * abs(f[i]) * auxVel[i]) / (B->mass)) * (B->blockedDOFs[i]);
		}

		double rotAccel = 0;
		double auxVelRot = 0;
		if (B->rotVel > 0) auxVelRot = 1; if (B->rotVel < 0) auxVelRot = -1;
		rotAccel = ((m - localDamping * abs(m) * auxVelRot) / (B->inertiaMoment)) * (B->blockedMDOFs);

		// update velocities
		if (nIter == 0) { //in order to initialize the leapfrog scheme (first iteration performs a half step on time)
			B->vel += linAccel * dtDEM * 0.5;
			B->rotVel += rotAccel * dtDEM * 0.5;
		}
		else {
